...
Qqwry.prototype.speed = function() {
  ...
  // var fs = require('fs');
  // var Path = typeof(this.dataPath) == "string" ? this.dataPath : pathDefined;
  // var callback = typeof arguments[arguments.length - 1] == "function" ? arguments[arguments.length - 1] : null;
  // this.ipFileBuffer = fs.readFileSync(Path);
  // // 缓存IP库文件;
  // return this;
}
...
function setIpFileString(Begin) {
  ...
  var toarr = [];
  var M;
  var isSuper = Boolean(this.ipFileBuffer);
...
function ipToInt(IP) {
  ...
  if (result) {
    ...
    ip = (parseInt(ip_Arr[0], 10) << 24 |
    parseInt(ip_Arr[1], 10) << 16 |
    parseInt(ip_Arr[2], 10) << 8 |
    parseInt(ip_Arr[3], 10)) >>> 0;
  } else if (/^\d+$/.test(IP) && (ip = parseInt(IP, 10)) >= 0 && ip <= 0xFFFFFFFF) {
    ip = Number(IP)
  } else {
...
function intToIP(INT) {
  if (INT < 0 || INT > 0xFFFFFFFF) {
    throw new Error("The number is not normal! >> " + INT);
  }
